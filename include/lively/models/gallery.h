...
// TokensModel.cs — Expiration serialized ISO-8601 like Newtonsoft's DateTime.
struct TokensModel {
    std::optional<std::string> access_token;   // AccessToken
    std::optional<std::string> refresh_token;  // RefreshToken
    std::optional<std::string> provider;
    std::string expiration_iso;                // Expiration, ISO-8601 string
...
inline TokensModel TokensModel::from_json(const Json& j) {
    ...
    if (j.contains("Expiration") && !j.at("Expiration").is_null()) t.expiration_iso = j.at("Expiration").get<std::string>();
    return t;
}
